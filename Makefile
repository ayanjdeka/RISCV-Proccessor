...
sim/dadda_dut_tb : $(SRCS)
	mkdir -p sim
	cd sim && vcs $(SRCS) $(VCS_FLAGS) -msg_config=../vcs_warn.config -l compile_dut.log -top dadda_dut_tb -o dadda_dut_tb
	bash check_compile_error.sh

sim/prefetch_dut_tb : $(SRCS)
	mkdir -p sim
	cd sim && vcs $(SRCS) $(VCS_FLAGS) -msg_config=../vcs_warn.config -l compile_dut.log -top prefetch_dut_tb -o prefetch_dut_tb
	bash check_compile_error.sh
...
.PHONY: run_dadda_tb
run_dadda_tb: sim/dadda_dut_tb
	rm -f sim/dump.fsdb
	rm -f sim/dump.fsdb.slock
	cd sim && ./dadda_dut_tb -l simulation_dut.log

.PHONY: run_prefetch_tb
run_prefetch_tb: sim/prefetch_dut_tb
	rm -f sim/dump.fsdb
	rm -f sim/dump.fsdb.slock
	cd sim && ./prefetch_dut_tb -l simulation_dut.log
	
.PHONY: run_cache_tb
run_cache_tb: sim/cache_dut_tb
	rm -f sim/dump.fsdb
	rm -f sim/dump.fsdb.slock
